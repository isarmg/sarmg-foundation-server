...
test('configured instance workspace, header actions, names and consumer override',async({page})=>{
  ...
  const actions=page.getByRole('group',{name:'全局操作'});
  ...
  await page.getByRole('button',{name:'选择实例 第二实例'}).click();
  await expect(page.getByRole('heading',{name:'Selected 2'})).toBeVisible();
  await actions.getByRole('button',{name:'刷新',exact:true}).click();
  await expect(page.getByText('Revision 1')).toBeVisible();
  await actions.getByRole('button',{name:'新建实例',exact:true}).click();
  ...
  await expect(actions.getByRole('button',{name:'新建实例',exact:true})).toBeFocused();
