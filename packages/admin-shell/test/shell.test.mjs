import test from"node:test";import assert from"node:assert/strict";import{readFile}from"node:fs/promises";test("shell owns sign in and request-safe failure copy",async()=>{const source=await readFile(new URL("../src/index.tsx",import.meta.url),"utf8");assert.match(source,/Administrator sign in/);assert.doesNotMatch(source,/localStorage|sessionStorage/);});test("font bootstrap is bounded and waits only for compact faces",async()=>{const source=await readFile(new URL("../src/index.tsx",import.meta.url),"utf8");assert.match(source,/Sarmg Maple Bootstrap/);assert.match(source,/FONT_BOOT_TIMEOUT_MS = 1200/);assert.doesNotMatch(source,/document\.fonts\.ready/);});
