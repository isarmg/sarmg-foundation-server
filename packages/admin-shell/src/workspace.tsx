...
export function HeaderNavigation({ children, label = "页面导航" }: { children: ReactNode; label?: string }) {
...
export function InstanceWorkspace({ instances, selected, select, label = "实例", showSidebar = true, children }: {
  ...
  return <div className={config.layout === "instances" ? `sarmg-instance-workspace${sidebarVisible ? "" : " sarmg-instance-workspace--full"}` : "sarmg-custom-workspace"}>
    {sidebarVisible && <aside className="sarmg-instance-sidebar" aria-label={label}><div className="sarmg-instance-list">{instances.map(item =>
      <Button key={item.id} title={item.name} aria-label={`选择实例 ${item.name}`} aria-pressed={selected === item.id} onClick={() => select(item.id)}><span>{item.name}</span></Button>
    )}</div></aside>}
    <section aria-label="实例详情与设置">{children}</section>
  </div>;
}
...
export function InstanceNameField({ onChange, onInput, ...props }: Omit<InputHTMLAttributes<HTMLInputElement>, "maxLength">) {
  const config = useContext(WorkspaceContext);
  const validate = (input: HTMLInputElement) => input.setCustomValidity(validInstanceName(input.value, config.instanceNameMaxCharacters) ? "" : `名称须为 1–${config.instanceNameMaxCharacters} 个字符，不能包含控制字符`);
