...
|---|---|---|---|
| Foundation 版本 | `0.7.9` | 根 `Cargo.toml`、`package.json`、各 crate/package、policy | 阻止 CI/发布，统一更新后重建 lock |
...
| Release tag | `v0.7.9` | Git tag | 一经发布不移动、不覆盖、不重建同版本 |
...
- tag `v0.7.9` 精确指向当前 HEAD，source revision 为完整小写 SHA。
...
├─ release-tree.json
└─ artifacts/
   ├─ sarmg-admin-web-0.7.9.tgz
   ├─ sarmg-contracts-0.7.9.tgz
   ├─ sarmg-design-tokens-0.7.9.tgz
   ├─ sarmg-http-client-0.7.9.tgz
   ├─ sarmg-release-tool-0.7.9.tar.gz
   ├─ state-contract.json
