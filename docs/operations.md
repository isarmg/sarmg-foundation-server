...
| Release tag | `v0.5.0` | Git tag | 一经发布不移动、不覆盖、不重建同版本 |
...
- 所有 crate 启用 workspace lint，内部 Rust dependency 精确 `=0.5.0`；
- 内部 npm build dependency 使用 `workspace:0.5.0`，peer 使用精确 `0.5.0`；
...
- tag `v0.5.0` 精确指向当前 HEAD，source revision 为完整小写 SHA。
...
└─ artifacts/
   ...
   ├─ sarmg-contracts-0.5.0.tgz
   ├─ sarmg-design-tokens-0.5.0.tgz
   ├─ sarmg-http-client-0.5.0.tgz
   ├─ sarmg-release-tool-0.5.0.tar.gz
   ├─ state-contract.json
