...
|---|---|---|---|
| Foundation 版本 | `0.3.0` | 根 `Cargo.toml`、`package.json`、各 crate/package、policy | 阻止 CI/发布，统一更新后重建 lock |
...
| License | Apache-2.0 | `LICENSE`、Cargo/npm metadata | 缺失即不发布 |
| Release tag | `v0.3.0` | Git tag | 一经发布不移动、不覆盖、不重建同版本 |
...
- Cargo/npm/policy 的版本均为 `0.3.0`；
- Rust `1.98.0`、Node `26.7.0`、pnpm `10.12.1` 的事实源一致；
- Rust workspace 恰好包含 6 个已知 crate，npm workspace 恰好包含 4 个已知 package；
- 所有 crate 启用 workspace lint，内部 Rust dependency 精确 `=0.3.0`；
- 内部 npm build dependency 使用 `workspace:0.3.0`，peer 使用精确 `0.3.0`；
...
- tag `v0.3.0` 精确指向当前 HEAD，source revision 为完整小写 SHA。
...
├─ release-tree.json
└─ artifacts/
   ├─ sarmg-admin-web-0.3.0.tgz
   ├─ sarmg-contracts-0.3.0.tgz
   ├─ sarmg-design-tokens-0.3.0.tgz
   ├─ sarmg-http-client-0.3.0.tgz
   ├─ sarmg-release-tool-0.3.0.tar.gz
   ├─ state-contract.json
