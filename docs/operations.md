...
|---|---|---|---|
| Foundation 版本 | `0.9.1` | 根 `Cargo.toml`、`package.json`、各 crate/package、policy | 阻止 CI/发布，统一更新后重建 lock |
...
| Release tag | `v0.9.1` | Git tag | 一经发布不移动、不覆盖、不重建同版本 |
...
- 所有 crate 启用 workspace lint，内部 Rust dependency 精确 `=0.9.1`；
- 内部 npm build dependency 使用 `workspace:0.9.1`，peer 使用精确 `0.9.1`；
...
- tag `v0.9.1` 精确指向当前 HEAD，source revision 为完整小写 SHA。
...
├─ release-tree.json
└─ artifacts/
   ├─ sarmg-admin-web-0.9.1.tgz
   ├─ sarmg-admin-shell-0.9.1.tgz
   ├─ sarmg-admin-ui-0.9.1.tgz
   ├─ sarmg-contracts-0.9.1.tgz
   ├─ sarmg-design-tokens-0.9.1.tgz
   ├─ sarmg-http-client-0.9.1.tgz
   ├─ sarmg-web-fonts-0.9.1.tgz
   ├─ sarmg-web-toolchain-0.9.1.tgz
   ├─ sarmg-release-tool-0.9.1.tar.gz
   ├─ state-contract.json
