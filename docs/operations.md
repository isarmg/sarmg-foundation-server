...
|---|---|---|---|
| Foundation 版本 | `0.8.3` | 根 `Cargo.toml`、`package.json`、各 crate/package、policy | 阻止 CI/发布，统一更新后重建 lock |
...
| Release tag | `v0.8.3` | Git tag | 一经发布不移动、不覆盖、不重建同版本 |
...
- 所有 crate 启用 workspace lint，内部 Rust dependency 精确 `=0.8.3`；
- 内部 npm build dependency 使用 `workspace:0.8.3`，peer 使用精确 `0.8.3`；
...
- tag `v0.8.3` 精确指向当前 HEAD，source revision 为完整小写 SHA。
...
├─ release-tree.json
└─ artifacts/
   ├─ sarmg-admin-web-0.8.3.tgz
   ├─ sarmg-admin-shell-0.8.3.tgz
   ├─ sarmg-admin-ui-0.8.3.tgz
   ├─ sarmg-contracts-0.8.3.tgz
   ├─ sarmg-design-tokens-0.8.3.tgz
   ├─ sarmg-http-client-0.8.3.tgz
   ├─ sarmg-web-fonts-0.8.3.tgz
   ├─ sarmg-web-toolchain-0.8.3.tgz
   ├─ sarmg-release-tool-0.8.3.tar.gz
   ├─ state-contract.json
