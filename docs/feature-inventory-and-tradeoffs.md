...
| FND-003 | 全组件统一版本 `0.3.0` | workspace version、package version、`foundation_policy.py` | 保障 | 中 | 同一 release 内无法确定可组合组件，lock 和支持矩阵失真 | repository policy 精确一致性检查 |
...
| FND-013 | Apache-2.0 元数据在 Cargo/npm 与 LICENSE 一致 | workspace/package manifests、`LICENSE` | 开发运维 | 低 | 发布物来源和使用权不可审计 | package metadata/tar 检查 |
...
| FND-328 | internal runtime peer使用精确0.3.0 | manifests/policy | 保障 | 高 | 依赖所有权隐藏或运行时解析另一代 | packed manifest检查 |
...
| FND-345 | tag必须精确`v0.3.0`且唯一指向HEAD | `verify_source` | 保障 | 高 | 资产版本/source/tag不能建立一一关系 | wrong/missing/multiple tag负例 |
...
`0.3.0` 只有在以下条件全部成立时才完成：6个crate和4个package身份一致；Rust/TS/Schema/fixture同构；
