...
20 个 crate 和 8 个 package 统一为 `0.8.3`。0.x 允许破坏性演进，但“不稳定”不等于可以静默漂移：每个版本仍是
...
Foundation workspace 中的 `workspace:0.8.3` 只用于 dev/build。发布 tar manifest 不能含 workspace 协议；真实
consumer必须同时安装所需tgz。
