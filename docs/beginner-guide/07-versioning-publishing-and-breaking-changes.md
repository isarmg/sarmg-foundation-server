...
6个crate和4个package统一为`0.3.0`。0.x允许破坏性演进，但“不稳定”不等于可以静默漂移：每个版本仍是
...
Foundation workspace中的`workspace:0.3.0`只用于dev/build。发布tar manifest不能含workspace协议；真实
consumer必须同时安装所需tgz。
