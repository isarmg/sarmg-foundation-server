...
完整流程。Foundation 是多个产品的安全下限；一次看似很小的 helper 修改可能同时改变 Server、Web、
...
产品中出现重复需求
├─ 只有一个真实消费者
│  └─ 留在产品；记录边界与测试，不预建通用框架
├─ 多个产品只有表面相似，失败/状态/安全语义不同
│  └─ 抽取更小 primitive，或继续留在产品
└─ 至少两个真实消费者具有相同边界
   ├─ 收集实际调用点、最强安全约束和删除后果
...
| 问题 | 合格证据 | 不合格答案 |
|---|---|---|
| 谁在使用 | 至少两个仓库的源码路径、测试和负责人 | “以后可能都会用” |
...
结论只有三种：进入 Foundation 的最小 primitive；保留在产品；等待第二消费者证据。若抽象要求统一产品
业务数据库、页面、进程生命周期、外部系统状态机或历史版本，则应缩小或拒绝，不把 Foundation 变成中央
平台。
...
├─ 产品执行 body/IP/account/global 限流
├─ 产品加载当前管理员记录
├─ verify_password（只接受当前 Argon2id PHC）
├─ random_token 分别生成 Session/CSRF 所需随机值
├─ 产品只持久化摘要、TTL、Session version 等自身字段
├─ 产品设置自己的 HttpOnly/Secure/SameSite Cookie
...
第一个同名 header。Cookie 名称、属性、Session TTL、撤销、并发上限和审计继续由产品实现。
...
强制消费者改用 pnpm。Dufs 是明确例外：原生 ES modules 被嵌入单 binary，但仍采用统一管理员 wire 和
Server 认证 primitive。
...
| 状态 | 含义 | 允许的证据 |
|---|---|---|
| `not-integrated` | 尚未采用 | `adopted_version=null`、空 package、无 verified commit |
| `integration-pending` | 本地 path/file 联调已进行，但不可变来源尚未完成 | 当前版本与直接组件，verified commit 可空 |
| `passing` | 不可变来源、独立 checkout 和完整产品门禁通过 | 必须有完整 40 位 `last_verified_commit` |
| `failing` | 已采用但当前验证失败 | 保留真实组件和失败状态，不伪装 passing |
