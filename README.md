...
当前工作区以 `0.7.4` 为版本基线，仅提供服务端及其管理 Web 的平台实现；P11/P12 客户端实现归 Client 仓库。
...
0.7.4 在 0.7.3 的无闪烁启动表面基础上加入首屏 Maple 子集，将静态管理 UI 的中西文字体压缩为每个字重一个请求。外壳只等待首屏字体，并在 1.2 秒内稳定选择 Maple 或系统回退，不再等待整个 `FontFaceSet`。
