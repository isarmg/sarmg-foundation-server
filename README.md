...
当前工作区以 `0.7.3` 为版本基线，仅提供服务端及其管理 Web 的平台实现；P11/P12 客户端实现归 Client 仓库。
...
0.7.3 在 0.7.2 的基础上修复初始会话恢复与字体加载闪烁，移除独立管理员列表页面，并保留右上角人物图标作为唯一账号设置入口。本轮五个服务端消费者统一使用 0.7.3 的不可变 Release 包并锁定 integrity；已有外壳与外观快照通过同步工具校验来源，不覆盖已发布制品。
