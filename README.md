...
Sarmg Foundation Server `0.9.1` 为 Sarmg 的 Rust/Axum 服务和管理 Web 提供共享基础能力，包括管理员认证、SQLite 状态、Schema 身份、运行时生命周期、文件系统安全、秘密封装、统一错误合同、设计令牌和 React 管理组件。
...
```toml
[dependencies]
sarmg-admin-core = { git = "https://github.com/isarmg/sarmg-foundation-server.git", rev = "<full-commit-sha>", version = "=0.9.1" }
```
...
正式发布对应不可变的 `v0.9.1` tag；版本更新时必须同步源码清单、锁文件和消费者验证记录。
