...
当前工作区以 `0.8.0` 为版本基线，仅提供服务端及其管理 Web 的平台实现；P11/P12 客户端实现归 Client 仓库。
...
| Foundation 版本 | `0.8.0` | Cargo/npm/package/release policy |
...
4. 普通 CI 只有 `contents: read`。只有精确 `v0.8.0` tag 的专用 release job 可获得 `contents: write`。
